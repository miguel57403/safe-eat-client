...
class OrdersActivity : AppCompatActivity() {
    ...
    private fun initAdapter() {
        ...
        val ordersItems = findViewById<RecyclerView>(R.id.orders_items)
        ordersItems.layoutManager = LinearLayoutManager(this)
...
class OrdersAdapter(private var data: ArrayList<Order>) :
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        val image = itemView.findViewById<ImageView>(R.id.order_item_image)
        val restaurant = itemView.findViewById<TextView>(R.id.order_item_restaurant)
        val date = itemView.findViewById<TextView>(R.id.order_item_date)
        val status_icon = itemView.findViewById<ImageView>(R.id.order_item_status_icon)
        val status_text = itemView.findViewById<TextView>(R.id.order_item_status_text)
        val products_number = itemView.findViewById<TextView>(R.id.order_item_products_number)

        fun bind(order: Order) {
            restaurant.text = order.restaurant
            date.text = order.date
            status_text.text = order.status
            setImageBase64(order.image)
            products_number.text = order.products_number.toString()
            when (order.status) {
                "Preparing" -> status_icon.setColorFilter(ContextCompat.getColor(itemView.context, R.color.orange_500))
                "Transporting" -> status_icon.setColorFilter(ContextCompat.getColor(itemView.context, R.color.orange_500))
                "Delivered" -> status_icon.setColorFilter(ContextCompat.getColor(itemView.context, R.color.green_500))
                else -> throw Exception("Invalid order status")
            }
        }

        private fun setImageBase64(base64: String) {
